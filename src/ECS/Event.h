...
using SubscriberPtrAllocator =
std::allocator_traits<Allocator>::template rebind_alloc<std::shared_ptr<BaseEventSubscriber>>;
...
std::pair<const TypeIndex, std::vector<std::shared_ptr<BaseEventSubscriber>, SubscriberPtrAllocator>>>;
using EntityAllocator = std::allocator_traits<Allocator>::template rebind_alloc<Entity>;
using SubscribersMap =
std::unordered_map<TypeIndex, std::vector<std::shared_ptr<BaseEventSubscriber>, SubscriberPtrAllocator>, std::hash<TypeIndex>, std::equal_to<TypeIndex>, SubscriberPairAllocator>;
...
class EventManager
{
    ...
    template <typename Event> void disconnect(EventSubscriber<Event>* subscriber)
    {
        auto index = getTypeIndex<Event>();
        auto it = mSubscribers.find(index);
