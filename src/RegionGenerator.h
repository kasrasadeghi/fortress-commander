...
public:
   ...
   * @detail The effect of modifying the lacunarity is subtle; you may need to play with the
   * lacunarity value to determine the effects. For best results, set the lacunarity to a number
   * between 1.5 and 3.5. http://libnoise.sourceforge.net/docs/classnoise_1_1module_1_1Perlin.html
   */
   ...
   * @detail The persistence value determines how quickly the amplitudes diminish for successive
   * octaves. The amplitude of the first octave is 1.0. The amplitude of each subsequent octave is
   * equal to the product of the previous octave's amplitude and the persistence value. So a
   * persistence value of 0.5 sets the amplitude of the first octave to 1.0; the second, 0.5; the
   * third, 0.25; etc. http://libnoise.sourceforge.net/docs/classnoise_1_1module_1_1Perlin.html
   */
   ...
   *
   * @tparam octaves The number of octaves control the amount of detail of the Perlin noise. Adding
   * more octaves increases the detail of the Perlin noise, but with the drawback of increasing the
   * calculation time. http://libnoise.sourceforge.net/docs/classnoise_1_1module_1_1Perlin.html
   *
  ...
  double generate(double noiseX, double noiseY = 0, double noiseZ = 0) {
    ...
    return std::max(0.,
                    std::min(1., result)); // clamp because there is no guarantee on output range
  }
};
